...
function showTab(tabId) {
  ...
  ['openskyTabBtn','aviationstackTabBtn','adsbexTabBtn'].forEach(btnId => {
...
document.addEventListener('DOMContentLoaded', () => {
  // Ensure served over HTTPS or localhost for geolocation
  if (!location.protocol.startsWith('http')) {
    console.warn('Geolocation requires HTTP/HTTPS context.');
  }
...
document.getElementById('marginSlider').addEventListener('input', e => {
  ...
  const feedback = margin <= 2.5 ? "🎯 Very strict (photography)"
...
function callTransitAPI(flights,uLat,uLon,uElev,bodyAz,bodyEl){
  fetch('/api/detect-transit',{method:'POST',headers:{'Content-Type':'application/json'},
    body: JSON.stringify({flights,userLat:uLat,userLon:uLon,userElev:uElev,bodyAz,bodyEl,margin})
  })
  .then(r=>r.json())
  .then(({matches,error})=>{
    const statusEl=document.getElementById('transitStatus');
    ...
    if(matches.length){
      const msgs=matches.map(m=>m.status==='Transit'
